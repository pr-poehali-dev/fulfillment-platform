...
export function PartnerDetailModal({ partner, onClose, onRequestQuote, isFavorite, onToggleFavorite }: {
  ...
  const header = (
    <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-100 flex-shrink-0 bg-white">
      {/* На мобиле — кнопка "назад", на десктопе — логотип */}
      <button onClick={onClose} className="md:hidden w-9 h-9 rounded-xl bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-600 flex-shrink-0">
        <Icon name="ChevronLeft" size={18} />
      </button>
      <div className="w-9 h-9 bg-navy-50 rounded-xl hidden md:flex items-center justify-center text-xl flex-shrink-0">{partner.logo}</div>
      <div className="flex-1 min-w-0">
      ...
      <button onClick={onClose} className="hidden md:flex w-8 h-8 rounded-lg bg-gray-100 hover:bg-gray-200 items-center justify-center text-gray-500 flex-shrink-0">
        <Icon name="X" size={14} />
      </button>
    </div>
  );
  ...
  return (
    <>
      {/* ── МОБИЛКА: fullscreen ── */}
      <div className="md:hidden fixed inset-0 z-[70] bg-white flex flex-col" onClick={(e) => e.stopPropagation()}>
        {header}
