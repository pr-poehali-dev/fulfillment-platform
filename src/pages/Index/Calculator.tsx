...
export function ContactsSection() {
  ...
  const [sent, setContactSent] = useState(false);
  const [submitting, setContactSubmitting] = useState(false);
  ...
  const FormContent = () => sent ? (
    <div className="text-center py-6">
      ...
      <button onClick={() => { setContactSent(false); setContactName(""); setContactEmail(""); setContactMessage(""); }}
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          ...
          <input value={name} onChange={e => setContactName(e.target.value)} placeholder="Иван Иванов" className={inputCls} />
        </div>
        <div>
          <label className="text-xs font-semibold text-gray-500 font-golos block mb-1">Email *</label>
          <input value={email} onChange={e => setContactEmail(e.target.value)} type="email" placeholder="you@company.ru" className={inputCls} />
        </div>
      </div>
      <div>
        <label className="text-xs font-semibold text-gray-500 font-golos block mb-1">Сообщение *</label>
        <textarea value={message} onChange={e => setContactMessage(e.target.value)} rows={4}
      ...
      <div className="flex items-center gap-4 flex-wrap pt-1">
        <Button onClick={handleSubmit}
          ...
          className="bg-navy-900 hover:bg-navy-800 text-white font-bold font-golos h-10 px-6 disabled:opacity-40">
          {submitting
    ...
    <section id="contacts" className="py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4">
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-2">
            <div className="h-px w-8 bg-gold-500" />
            <span className="text-gold-600 text-xs font-medium tracking-widest uppercase font-ibm">Поддержка</span>
          </div>
        ...
        {/* Форма — десктоп */}
        <div className="hidden lg:block bg-gray-50 border border-gray-100 rounded-2xl p-6">
          <h3 className="font-golos font-bold text-lg text-navy-900 mb-4 flex items-center gap-2">
            <Icon name="LifeBuoy" size={18} className="text-navy-700" />
            Написать в поддержку
          </h3>
          <FormContent />
        </div>

        {/* Кнопка для мобильного — открывает bottom sheet */}
        <div className="lg:hidden">
          <Button onClick={() => setOpen(true)}
            className="w-full bg-navy-900 hover:bg-navy-800 text-white font-bold font-golos h-12 rounded-xl">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-100 px-5 py-4 flex items-center justify-between rounded-t-2xl">
              <div className="flex items-center gap-2">
                ...
                <span className="font-golos font-bold text-navy-900">Написать в поддержку</span>
              </div>
              <button onClick={() => setOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 transition-colors">
                <Icon name="X" size={14} className="text-gray-600" />
              </button>
            </div>
            {/* Drag handle */}
            <div className="flex justify-center pt-3 pb-1 absolute top-0 left-0 right-0 pointer-events-none">
